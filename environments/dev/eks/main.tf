data "terraform_remote_state" "vpc" {
  ...
  config = {
    bucket = "eks-demo-101"
    key    = "terraform-state/dev/core.tfstate"
    region = "eu-central-1"
  }
}
...
// Create EKS cluster in Dev environment
...
module "eks_cluster" {
  ...
  eks_subnet_ids  = [ 
    ...
    data.terraform_remote_state.vpc.outputs.eks_subnet_priv_1c,
    data.terraform_remote_state.vpc.outputs.eks_subnet_priv_1d
  ]                      

}


// Create worker nodes for Dev EKS cluster
